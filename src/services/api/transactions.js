...
import { callContractMethod } from './contracts.js';
 ...
 * @param {string} hash - Transaction hash
 * @returns {Promise<Object|null>} - Decoded transaction data or null if not found
 */
const getDecodedTransaction = async (hash) => {
  try {
    console.log(`Fetching decoded transaction data for hash: ${hash}`);
    const response = await api.get(`/transactions/${hash}/decoded`);
    return processApiData(response.data);
  } catch (error) {
    console.error(`Error fetching decoded transaction data for hash ${hash}:`, error);
    return null;
  }
};
